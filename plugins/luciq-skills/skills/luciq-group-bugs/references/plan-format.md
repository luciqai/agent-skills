...
WRITE SUMMARY
  ...
  ⚠ Merging moves each duplicate's occurrences into its master and OVERWRITES the
    duplicate's status & priority with the master's. No bulk undo on the server
    (this skill can undo merges it made this session).
...
Failures: none
Undo available: unmark the 3 bugs merged this session? (y/n)
```
