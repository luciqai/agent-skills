...
7. *(optional)* When the EM also needs span-level performance, the per-span `apm_*` chain via direct JSON-RPC (stripped from the client by the server-side schema limitation but read-only — see the constraint note in `metrics-glossary.md`): `apm_list_groups` (metric `network` sorted by `failure_rate`, `screen_loading` sorted by `p95`) → `apm_group_view(group_uuid, views=[summary, failure_rate, dimensions])` for the worst group's apdex / p95 / failure split → `apm_occurrence(group_uuid, selector=worst|list)` for one occurrence's per-stage span timeline (the dominant slow stage). Read-only; label every figure as sourced via a direct call.

**Drill-down included:** full stacktraces, one occurrence's detail, the diagnostics aggregate, and (optionally) the per-span APM group view + occurrence timeline. Raw numbers are fine and expected.
...
- **Performance lives at the `apm` aggregate.** Every tier that gets performance gets it as apdex / p95 / drop-off summaries from `app_insights.apm`, not per-span traces. The per-span `apm_*` tools are stripped from the MCP client by the server-side schema limitation and are `luciq-debug`'s job; an EM (or PM, for an owned flow) readout may optionally reach the read-only `apm_list_groups` / `apm_group_view` / `apm_occurrence` chain via a direct JSON-RPC call for a slowest-endpoint / slowest-screen group view and a per-stage span timeline, labeled as sourced that way. No other tier descends below the aggregate.
