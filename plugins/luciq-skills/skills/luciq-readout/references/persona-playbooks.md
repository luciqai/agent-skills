...
7. *(optional)* When the EM also needs span-level performance, the per-span `apm_*` chain via direct JSON-RPC (stripped from the client by AI-94 but read-only — see the constraint note in `metrics-glossary.md`): `apm_list_groups` (metric `network` sorted by `failure_rate`, `screen_loading` sorted by `p95`) → `apm_group_view(group_uuid, views=[summary, failure_rate, dimensions])` for the worst group's apdex / p95 / failure split → `apm_occurrence(group_uuid, selector=worst|list)` for one occurrence's per-stage span timeline (the dominant slow stage). Read-only; label every figure as sourced via a direct call.
