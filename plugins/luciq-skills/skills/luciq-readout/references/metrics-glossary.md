...
- **Per-span APM detail** beyond the `apm` section's aggregates is not the readout's primary source. The `apm_*` tools that expose it are stripped from the MCP client by the server-side schema limitation (see the constraint note above) and are `luciq-debug`'s territory; all three are read-only and reachable via the confirmed direct JSON-RPC path, but a readout reaches them only when a persona truly needs span depth, and labels the result accordingly. Session replay is not exposed to these tools.
